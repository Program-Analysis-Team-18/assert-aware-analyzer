...
def add_comments_to_file(class_file: Classes):
    ...
    def add_assertion_prefix():
        ...
        if avg >= 2:
            ...
            prefix = f"// The file has on average {avg} assertions per method. This is not sufficient\n"
    ...
    def suggest_assertions():
        """
        suggest_assertions will add an assertion based on the output of the fuzzer.
        """
        lines = class_file_path.read_text().splitlines(keepends=True)
...
def validate_mapping(assertion_mapping):
    ...
    assert set(current_classes) == set(
        assertion_classes), "mapping is not valid"
